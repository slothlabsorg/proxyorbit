...
// Allow ?updater=1 or ?mockUpdate=1 to preview the modal with fake data
function getMockParam() {
  try {
    const p = new URL(window.location.href).searchParams
    return p.get('updater') === '1' || p.get('mockUpdate') === '1'
  } catch { return false }
}
...
function getMockVersion() {
  try {
    const p = new URL(window.location.href).searchParams
    return p.get('mockUpdateVersion') ?? '1.1.0'
  } catch { return '1.1.0' }
}

const MOCK_UPDATE: UpdateInfo = {
  version: getMockVersion(),
  body: `## What's new in v${getMockVersion()}\n\n- HTTPS MITM inspection improvements\n- Request replay outside the proxy stream\n- Intercept queue: pause and edit multiple requests\n- System proxy auto-configure with orphan cleanup\n- WebSocket CONNECT tunnel capture`,
            ...
            >
              ...
              {state.status === 'available' && (
                <p className="text-center text-text-muted/60 text-[10px] px-4 pb-2 leading-relaxed">
                  ProxyOrbit is in early release — we're actively building it and shipping improvements constantly.
                </p>
              )}
...
function LogoOrIcon() {
  ...
  return (
    <img
      src="/images/logo-proxyorbit.PNG"
      alt="ProxyOrbit"
